...
pca_quality <- PCA(quality_complete[, c(6, 8:14)], 
                   scale.unit = TRUE, 
                   graph = FALSE)

summary(pca_quality) # 35.43%, 19.86% 


# PERMANOVA for validation

scaled_quality <- scale(quality_complete[, c(6, 8:14)])
...
pca_quality_leaf <- PCA(quality_leaf[, c(6, 8:14)], 
                        scale.unit = TRUE, 
                        graph = FALSE)
...
scaled_quality_leaf <- scale(quality_leaf[, c(6, 8:14)])
...
pca_quality_root <- PCA(quality_root[, c(6, 8:14)], 
                        scale.unit = TRUE, 
                        graph = FALSE)
...
scaled_quality_root <- scale(quality_root[, c(6, 8:14)])
...
## Create data tibble for plots ----

microcosm_master_plot <- 
  microcosm_master %>%
  mutate(
    dom_organ = str_c(
      dom_status,
...
microcosm_master_plot$dom_organ <-
  parse_factor(
    microcosm_master_plot$dom_organ,
...
# Calculate mean mass loss per organ, 
# since we know dom_status is not significant
# to show it on Figure 4
...
microcosm_master_plot$sp_richness <- factor(
  as.character(microcosm_master_plot$sp_richness),
  levels = c("1", "2", "4"),
  labels = c("1 species",
             "2 species",
             "4 species")
...
microcosm_master_plot$dom_shape <- as.numeric(
  microcosm_master_plot$dom_status
)

microcosm_master_plot$dom_shape[microcosm_master_plot$dom_shape == 1] <- 24
microcosm_master_plot$dom_shape[microcosm_master_plot$dom_shape == 2] <- 21

microcosm_master_plot$dom_shape <- as.factor(
  microcosm_master_plot$dom_shape
)
...
# Draws a PCA plot leaves + roots
pca_plot_both <- function(original_data,
                          pca_result){
  ...
  data_ord <- as_tibble(cbind(organ_pca, dim_1, dim_2))
  
  
  # Name elements of the tibble to simplify syntax when programming
  names(data_ord) <- c("organ_pca", "dim_1", "dim_2")
  
  
  # Parse factor
  data_ord$dom_organ <- parse_factor(data_ord$organ_pca)
...
# Draws a PCA plot leaves or roots
...
p2_1 <- pca_plot_both(quality_complete,
                      pca_quality
                      ) +
